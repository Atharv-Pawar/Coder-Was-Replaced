...
class Editor:
    ...
    def update(self, dt: float, script_engine=None, progression=None) -> None:
    ...
    def _draw_progress_panel(self, surface: pygame.Surface, progression) -> None:
        panel_y = self._code_y + self._code_h
        ...
        if progression is None:
            placeholder = self.status_font.render(
                "Progression loading...", True, c.COLOR_EDITOR_GUTTER_TEXT)
            surface.blit(placeholder, (8, panel_y + 8))
            return

        pad_x = 8
        pad_y = panel_y + 7

        # ── Row 1: level title + XP counter ─────────────────────────────
        level = progression.level
        next_unlock = progression.next_unlock
        level_surf = self.font.render(f">> {level.title}", True, c.COLOR_LEVEL_TEXT)
        surface.blit(level_surf, (pad_x, pad_y))

        xp_text = f"XP: {progression.xp}"
        if next_unlock:
            xp_text += f" / {next_unlock.xp_required}"
        xp_surf = self.status_font.render(xp_text, True, c.COLOR_LEVEL_XP_TEXT)
        surface.blit(xp_surf, (self.W - xp_surf.get_width() - pad_x, pad_y + 1))

        # ── Row 2: XP progress bar ───────────────────────────────────────
        bar_y = pad_y + self.LH + 2
        bar_w = self.W - pad_x * 2
        bar_h = 9
        ratio = progression.xp_progress_ratio

        pygame.draw.rect(surface, c.COLOR_XP_BAR_BG,
                         pygame.Rect(pad_x, bar_y, bar_w, bar_h), border_radius=4)
        if ratio > 0:
            fill_color = c.COLOR_XP_BAR_FILL2 if ratio > 0.75 else c.COLOR_XP_BAR_FILL
            fill_w = max(6, int(bar_w * ratio))
            pygame.draw.rect(surface, fill_color,
                             pygame.Rect(pad_x, bar_y, fill_w, bar_h), border_radius=4)
        pygame.draw.rect(surface, c.COLOR_PROGRESS_DIVIDER,
                         pygame.Rect(pad_x, bar_y, bar_w, bar_h), width=1, border_radius=4)

        # ── Row 3: next unlock hint ──────────────────────────────────────
        hint_y = bar_y + bar_h + 4
        if next_unlock:
            need = progression.xp_to_next_unlock
            hint = f"Next: {next_unlock.fn_name}()  --  {need} XP away"
            hint_surf = self.status_font.render(hint, True, c.COLOR_NEXT_UNLOCK)
        else:
